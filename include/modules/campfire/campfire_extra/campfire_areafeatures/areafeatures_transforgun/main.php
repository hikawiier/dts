...
{	
	...
	function repair_gun($r_sk)
	{
		...
		{
			$log.="<span class='red'>你所装备的武器不属于枪械，无法对其进行改造！</span><br>";
			return;
		}
		...
		for($i=1;$i<=6;$i++)
		{
			if((in_array(${'itm'.$i},array_keys($repair_item_effect)) || in_array(${'itmk'.$i},$gun_weapon_arr)) && ${'itms'.$i}>0)
			{
				if(in_array(${'itmk'.$i},$gun_weapon_arr)) $rub_gun_flag = true;
				$rubbish_flag = $i;
				break;
			}
		}
		if($rubbish_flag)
		{
			...
			if($r_sk=='addwepe')
			{
				...
				elseif ($rub_gun_flag) $rub_add_wepe = round($rub['itme']/2);//枪加值等于效果/2
			...
			else
			{
				...
				elseif ($rub_gun_flag) $rubbish_add_obbs = round($rub['itme']/15);//枪械加值等于效果/15
