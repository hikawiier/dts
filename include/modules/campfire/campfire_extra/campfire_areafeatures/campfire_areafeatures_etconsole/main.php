...
namespace campfire_areafeatures_etconsole
{
	function init()
	{
	}
	function act()
	{
		...
		eval(import_module('sys','player','logger','campfire_areafeatures_etconsole','input','weather','npc','itemmain'));
		/*==========无月之影特殊功能：campfire_areafeatures_etconsole菜单部分开始==========*/
		if($mode == 'lp_campfire_areafeatures_etconsole')
		{
			if($pls!=0)
			{
				$log.="该地图没有campfire_areafeatures_etconsole功能，如果遇到了BUG，请您将这句话转述给管理员。<br>";
				return;
			}
			$check_campfire_areafeatures_etconsole_flag = 0;
			$check_campfire_areafeatures_etconsole_flag = check_campfire_areafeatures_etconsole();
			...
			{
				ob_clean();
				include template(MOD_campfire_areafeatures_etconsole_LP_campfire_areafeatures_etconsole_CWTH);
				$cmd = ob_get_contents();
				ob_clean();
				return;
			}
			...
			{
				campfire_areafeatures_etconsole_bancombo();
			}
			elseif($command=='findthings')
			{
				ob_clean();
				include template(MOD_campfire_areafeatures_etconsole_LP_campfire_areafeatures_etconsole_FIND);
				$cmd = ob_get_contents();
				ob_clean();
				return;
			}
			...
			{
				campfire_areafeatures_etconsole_extractnpc();
			}
			elseif($command=='copyhack')
			{
				campfire_areafeatures_etconsole_copyhack();
			}
			else
			{
				$log.="campfire_areafeatures_etconsole菜单指令选择错误，如果遇到了BUG，请您将这句话转述给管理员。<br>";
			}
		}
		if($campfire_areafeatures_etconsole=='campfire_areafeatures_etconsole_changewth')
		{
			if($pls!=0)
			{
				$log.="该地图没有campfire_areafeatures_etconsolecwth功能，如果遇到了BUG，请您将这句话转述给管理员。<br>";
				return;
			}
			$check_campfire_areafeatures_etconsole_flag = 0;
			$check_campfire_areafeatures_etconsole_flag = check_campfire_areafeatures_etconsole();
			...
			elseif($command=='cwth')
			{
				campfire_areafeatures_etconsole_cwth($iweather);
			}
			else
			{
				$log.="campfire_areafeatures_etconsolecwth菜单指令选择错误，如果遇到了BUG，请您将这句话转述给管理员。<br>";
			}
		}
		elseif($campfire_areafeatures_etconsole== 'campfire_areafeatures_etconsole_findthings')
		{
			if($pls!=0)
			{
				$log.="该地图没有campfire_areafeatures_etconsolecwth功能，如果遇到了BUG，请您将这句话转述给管理员。<br>";
			...
			elseif($command=='cfindpc' || $command=='cfinditm'  || $command=='cfindtrap')
			{
				...
				campfire_areafeatures_etconsole_find($finding,$findingname,0);
			}
			elseif($command=='cfindnpc')
			{
				$finding = substr($command,1);
				campfire_areafeatures_etconsole_find($finding,$findingname,$findingnpctype);
			}
			else
			{
				$log.="campfire_areafeatures_etconsolefind菜单指令选择错误，如果遇到了BUG，请您将这句话转述给管理员。<br>";
			}
		}
		elseif($campfire_areafeatures_etconsole=='campfire_areafeatures_etconsole_mob')
		{
			...
			elseif($mob_flag && (($command=='gsc_addarea') || ($command =='gsc_hack') || ($command =='gsc_radar')))
			{
				...
				campfire_areafeatures_etconsole_mob($gsc_order,$radar_mmn);
			}
			else
			{
				$log.="你身上没有控制台的子机！<br>";
		...
		/*==========无月之影特殊功能：campfire_areafeatures_etconsole菜单部分结束==========*/
		$chprocess();
	}
	/*==========无月之影特殊功能：campfire_areafeatures_etconsole功能部分开始==========*/
	function campfire_areafeatures_etconsole_extractnpc()
	{
		if (eval(__MAGIC__)) return $___RET_VALUE;	
		eval(import_module('sys','player','itemmain','logger','addnpc','npc','campfire_areafeatures_etconsole'));
	...
	function campfire_areafeatures_etconsole_copyhack()
	{
		if (eval(__MAGIC__)) return $___RET_VALUE;	
	...
	function campfire_areafeatures_etconsole_bancombo()
	{
		if (eval(__MAGIC__)) return $___RET_VALUE;	
		eval(import_module('sys','player','itemmain','logger'));	
	...
	function campfire_areafeatures_etconsole_mob($c_order,$c_radar)
	{
		if (eval(__MAGIC__)) return $___RET_VALUE;	
	...
	function campfire_areafeatures_etconsole_find($findtype,$findnm,$npctype)
	{
		if (eval(__MAGIC__)) return $___RET_VALUE;	
	...
	function campfire_areafeatures_etconsole_cwth($wth)
	{
		if (eval(__MAGIC__)) return $___RET_VALUE;	
	...
	function check_campfire_areafeatures_etconsole()
	{
		if (eval(__MAGIC__)) return $___RET_VALUE;	
		eval(import_module('sys','player','logger','itemmain','campfire_areafeatures_etconsole'));		
		$crm_lose_flag = false;
	...
	/*==========无月之影特殊功能：campfire_areafeatures_etconsole功能部分结束==========*/
}
