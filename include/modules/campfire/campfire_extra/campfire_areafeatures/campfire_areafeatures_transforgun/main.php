...
namespace campfire_areafeatures_transforgun
{
	function init()
	{
	}
	function act()
	{
		...
		eval(import_module('sys','player','logger','campfire_areafeatures_transforgun','input'));
		/*==========Fargo前基地特殊功能：campfire_areafeatures_transforgun菜单部分开始==========*/
		if($mode == 'lp_campfire_areafeatures_transforgun')
		{
			if($command == 'menu')
			{
			...
			{
				$log.="campfire_areafeatures_transforgun指令选择错误，如果遇到BUG，请联系管理员。<br>";
				return;
			}
			return;
		}
		if($mode == 'campfire_areafeatures_transforgun_confirm')
		{
			if($command == 'menu')
			{
			...
			{
				$log.="campfire_areafeatures_transforgun确认指令选择错误，如果遇到BUG，请联系管理员。<br>";
				return;
			}
			return;
		}
		/*==========Fargo前基地特殊功能：campfire_areafeatures_transforgun菜单部分结束==========*/
		$chprocess();
	}
	/*==========Fargo前基地特殊功能：campfire_areafeatures_transforgun功能部分开始==========*/
	...
	{
		if (eval(__MAGIC__)) return $___RET_VALUE;
		eval(import_module('sys','player','itemmain','logger','campfire_areafeatures_transforgun','input'));
		...
		ob_clean();
		include template(MOD_campfire_areafeatures_transforgun_LP_campfire_areafeatures_transforgun_CONFIRM);
		$cmd = ob_get_contents();
		ob_clean();
		return;
	}
	...
	/*==========Fargo前基地特殊功能：campfire_areafeatures_transforgun功能部分结束==========*/
}
