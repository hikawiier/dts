...
namespace weathermain
{
	...
	function move_to_area($moveto)	//天气对移动的特效
	{
		...
		if($weather == 11)	//龙卷风
		{
			...
			{
				$pls = rand($areanum+1,(sizeof($plsinfo)-sizeof($hidden_arealist))-1);$pls=$arealist[$pls];
			}
	...
	function rs_game($xmode = 0) 	//开局天气初始化
	{
		...
		if ($xmode & 2) 
		{
			foreach(array_keys($plsinfo) as $area)
			{
...
/*	function init_playerdata()
	...
	function parse_news($nid, $news, $hour, $min, $sec, $a, $b, $c, $d, $e)
	{
		...
		if($news == 'wthchange') 
			return "<li id=\"nid$nid\">{$hour}时{$min}分{$sec}秒，<span class=\"lime\">{$a}使用了{$c}，天气变成了{$wthinfo[$b]}！</span><br>\n";
		if($news == 'wthfail') 
			return "<li id=\"nid$nid\">{$hour}时{$min}分{$sec}秒，<span class=\"lime\">{$a}使用了{$c}，但是恶劣的天气并未发生改变！</span><br>\n";
		if($news == 'syswthchg') 
			return "<li id=\"nid$nid\">{$hour}时{$min}分{$sec}秒，<span class=\"lime\">奇迹和魔法都是存在的！当前天气变成了{$wthinfo[$a]}！</span><br>\n";
		...
		if($news == 'death17') 
		{
			...
			return "<li id=\"nid$nid\">{$hour}时{$min}分{$sec}秒，<span class=\"yellow\">$a</span>被<span class=\"red\">冰雹砸死</span>{$e}";
		}
